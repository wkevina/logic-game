...
class WalkerSystem(ecs.System):
    ...
    def update(self, dt, entity_manager):
        ...
        for e_id, jumper in pairs:
            ...
            input = pi.input['HORIZONTAL_1']
            
            if input:
                # test to see if we want to go in the direction we're already moving
                if copysign(1, input) == copysign(1, vel.v_x) or abs(vel.v_x) == 0:
                    vel.v_x += input * jumper.acc * dt
                    if abs(vel.v_x) > jumper.walk:
                        vel.v_x = jumper.walk * vel.v_x / abs(vel.v_x)
                else:
                    input = 0
            if abs(vel.v_x) > 0 and not input:
                old = vel.v_x
